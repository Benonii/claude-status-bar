...
The icon is the **walking Claude creature** (pixel art): it walks — legs moving,
eyes darting right — while Claude is working, and stands still when idle.

| state        | trigger (hook)        | panel widget |
|--------------|-----------------------|--------------|
| `thinking`   | UserPromptSubmit / PostToolUse | creature walking + "Thinking…  m:ss" |
| `tool`       | PreToolUse            | creature walking + labelled ("Editing", "Running command", …) + timer |
| `permission` | Notification (permission) | creature + "Awaiting permission" |
| `waiting`    | Notification (other)  | creature + "Waiting for you" |
| `done`/`idle`| Stop / no session     | dimmed, standing creature, no text |

Left-click the widget for a small popup with project + elapsed detail.

The sprite is generated by `tools/gen-claude-sprite.py` into
`plasmoid/contents/icons/claude-walk.png` (a 4-frame, 16×14-per-frame sheet) — a
hand-made approximation of Anthropic's mascot. To use the **official** asset, drop a
4-frame 16×14 sheet at that path (or adjust `frameCount`/`frameWidth`/`frameHeight`
in `main.qml`). The SNI tray fallback still uses the procedural spark.
