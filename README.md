...
`plasmoid/contents/icons/claude-walk.png` (a 4-frame, 13×12-per-frame sheet)
modelled on Anthropic's "Claw'd" mascot. To use the **official** asset, drop a
4-frame sheet at that path and match `frameCount`/`frameWidth`/`frameHeight` in
`main.qml`. The SNI tray fallback still uses the procedural spark.
